...
import styles from './CashflowStatement.module.scss';
...
class CashFlowStatement extends Component {
    ...
    constructor(props) {
     ...
     processData(stocksData) {
        let metrics = this.sortStatementByPosition(stocksData.cashflowStatement[0]);
        let metricsRowData = [];
    ...
    getActiveColumns(stocksData) {
      let ratiosByFiscalPeriod = stocksData.cashflowStatement;
    ...
    getDataToShow(metricsOrdered,stocksData,isInitialData = true) {
        let cashFlowStatements = stocksData.cashflowStatement;
        ...
        for(let i = 0; i < metricsOrdered.length; i++) {
           ...
           if(isInitialData && metricsOrdered[i].intend === 1) {
             for(let j = 0; j < cashFlowStatements.length; j++) {
                if (cashFlowStatements[j].hasOwnProperty(metric)) {
                   let fiscalPeriod = this.setToCamelCase(cashFlowStatements[j].fiscalPeriod);
                   if(currentRatioByFiscalPeriod.metricFiscalPeriod === undefined) {
                        currentRatioByFiscalPeriod.metricFiscalPeriod = cashFlowStatements[j][metric].name;
                   ...
                   currentRatioByFiscalPeriod[fiscalPeriod] = cashFlowStatements[j][metric].value;
                }
             }
